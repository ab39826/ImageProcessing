...
void createCookBook(Mat staveReg, int rightIndex);
void removeStaves(Mat image, Vector<int> firstVec, Vector<int> endVec, Mat resultImage); 
void findNotes(Mat staveReg, Vector<float> staveLoc);
...
namedWindow( "Before Image", CV_WINDOW_AUTOSIZE );
 ...
 while(barLineIndex < avgrows.size()) {
	 ...
	 Mat testMat = cleanImage.rowRange(startrow, endrow);
...
RNG rng(12345);
  ...
  for( int i = 0; i< contours.size(); i++ )
     {
		...
		// can continuously apply localminMax to the template until it falls below a certain threshold
		 ...
		 //for any rectangle that you find a match above a certain threshold, record a rectangle with coded colors, but eventually
		 ...
		 if(boundRect[i].area()>=templateQuarter.size[0]*templateQuarter.size[1] &&boundRect[i].area()<=3*(templateQuarter.size[0]*templateQuarter.size[1]) ){
			 ...
			 //construct a new Mat from boundRect coordinates in staveRegion
  ...
  /// Show me what you got
   ...
   if(maxVal>NOTE_THRESHOLD){
	  ...
	  rectangle( staveReg, Point(matchLoc.x + boundRect[i].tl().x, boundRect[i].tl().y + matchLoc.y), Point(matchLoc.x + boundRect[i].br().x, boundRect[i].br().y + matchLoc.y), Scalar::all(0), 2, 8, 0 );
